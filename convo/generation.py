...
def ollama_debate(
    ...
    opponent_previous_output_key = f"{opp_name}_output_{rounds - 1}"

    # Keys for the current model's raw and structured output for the current round
    current_raw_output_key = f"{curr_name}_raw_output_{rounds}"
    current_structured_output_key = f"{curr_name}_output_{rounds}"
    ...
    for i, s in tqdm(enumerate(all_results)):
        ...
        if (
            current_structured_output_key not in s
            and opponent_previous_output_key in s
            and s[opponent_previous_output_key] is not None
        ):
            opponent_structured_output = s[opponent_previous_output_key]

            # Extract details from opponent's structured output
            opponent_exp = opponent_structured_output.get(
            ...
            debate_instruc = [
                f"\n\nYour opponent, {opp_name}, has provided the following solution:",
            ...
            # TODO: separate
            prompt = prepare_context(test_samples[i], debate_instruc)
            ...
            current_structured_output_key not in s
        ):  # If opponent_previous_output_key is missing or invalid
            s[current_raw_output_key] = {
                "response": "No opponent output to debate.",
                "gold": test_samples[i]["answer"].strip().lower(),
            }
            s[current_structured_output_key] = (
                invalid_result()
            )  # Mark as invalid due to lack of debate context
